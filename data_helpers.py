...
class DataHelper(object):
    ...
    def load_data_and_labels(self):
        ...
        y_sentiment = pd.get_dummies(data_df['Clinical_sentiment'], prefix="Clinical_sentiment").values

        data_df['Clinical'] = data_df['Clinical'].str.replace(' ', '')
        f = lambda text: self.clean_str(text).split(' ')
        text_df = (data_df["Clinical"] + " " + data_df["Text"]).map(f)
